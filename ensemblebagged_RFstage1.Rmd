...
create_ensemble <- function(dat, particvfoldK, cvK){
  ...
  for(r in 1:cvK){
    ...
    for(k in 1:nrow(grd)){
      ...
      # for train
      ROCF_train <- roc(perf$trainprob$obs, perf$trainprob$mass, col="#000086", main=paste0("ROC T=",T," D=",D," cv=",r))
      ...
      # for test
      #par(new=TRUE)
      ROCF_test <- roc(perf$testprob$obs, perf$testprob$mass, col="#860000", main=paste0("ROC T=",T," D=",D," cv=",r))
...
ROCF_test <- plot.roc(resamROC_test$obs, resamROC_test$mass, col="#860000", lty=2,
                      main="ROC for stage1 max cvFolds")
print(ROCF_train$auc)
print(ROCF_test$auc)
legend("bottomright", 
